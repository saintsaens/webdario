...
export const createUser = async (username, hashedPw, role, sessionStartTime, lastActivityTime) => {
    const query = `
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id, username, role, session_start_time, last_activity_time, previous_time_spent;
    `;
...
export const getUserById = async (id) => {
    const query = `
        SELECT id, username, role, session_start_time, last_activity_time, previous_time_spent
        FROM ${tableName}
        WHERE id = $1;
    `;
...
export const updateUser = async (id, { username, hashedPw, role, sessionStartTime, lastActivityTime, previousTimeSpent }) => {
    const query = `
        UPDATE ${tableName}
        SET
            ...
            previous_time_spent = COALESCE($6, previous_time_spent)
        WHERE id = $7
        RETURNING id, username, role, session_start_time, last_activity_time, previous_time_spent;
    `;
    const { rows } = await db.query(query, [username, hashedPw, role, sessionStartTime, lastActivityTime, previousTimeSpent, id]);
    return rows[0];
};
...
export const addTimeSpent = async (id, timeToAdd) => {
    const query = `
        UPDATE ${tableName}
        SET previous_time_spent = COALESCE(previous_time_spent, 0) + $1
        WHERE id = $2
        RETURNING id, username, role, session_start_time, last_activity_time, previous_time_spent;
    `;
